...
def next_payment(debt_data_dict, payment_plan_data_dict):
    for debt_data in debt_data_dict:
        ...
        if is_in_payment_plan:
            for payment_plan_data in payment_plan_data_dict:
                if debt_data.get('id') == payment_plan_data.get('debt_id'):
                    amount_to_pay = payment_plan_data.get('amount_to_pay')
                    installment_amount = payment_plan_data.get('installment_amount')
                    remaining_amount = amount_to_pay - installment_amount
                    ...
                    next_payment_due_date_str = next_payment_date.strftime("%Y-%m-%d %H:%M:%S")
